...
function LogMessage(id,time,nick,user_colour,msg,color)
{
    var html = "";
    if(time) html += "<span class='date'>"+time+"</span>&nbsp;&nbsp;";
    if(nick) html += "[<a href='#' style='color: #"+user_colour+"' onclick=\"javascript:MessageEdit.To('"+addslashes(nick)+"'); return false;\">"+nick+"</a>]&nbsp;&nbsp;";
...
{
    LogEvent("<span class='date'>"+time+"</span> "+userJoined+" [<a href='#' onclick=\"javascript:MessageEdit.To('"+addslashes(nick)+"'); return false;\">"+nick+"</a>]");
}

function LogUserLeft(id,time,nick)
{
    LogEvent("<span class='date'>"+time+"</span> "+userLeft+" [<a href='#' onclick=\"javascript:MessageEdit.To('"+addslashes(nick)+"'); return false;\">"+nick+"</a>]");
}

function SetUsers(users)
{
    document.getElementById("users").innerHTML="<h4>"+nowInChat+" <b>"+users.length+" </b></h4>";
    for(var i = 0; i < users.length; i++)
    {
        var obj = document.createElement('span');
        obj.innerHTML="";
        obj.innerHTML+="&nbsp;<a href='#' onclick=\"javascript:MessageEdit.To('"+addslashes(users[i].name)+"', true); return false;\"><img src='/f/ext/infants/infchat/media/private.png' height='10px' width='10px' border='0' title='"+myPrivate+"' /></a>&nbsp;";
        obj.innerHTML+="<a class=\"user-block\" style=\"color: #"+users[i].color+"\" href='#' onclick=\"javascript:MessageEdit.To('"+addslashes(users[i].name)+"'); return false;\">"+users[i].name+"</a>";
...
function RefreshChat()
{
    ...
    $.ajax(
        {
            type: 		"POST",
            url: 		"/f/infchat", //build={BUILD_TIME}",
